// Generated by Bisonc++ V6.01.00 on Wed, 03 Apr 2019 21:40:01 +0200
...
PI__ const s_productionInfo[] = 
{
     ...
     {262, 2}, // 4: line (errTok__) ->  errTok__ '\x0a'
     {262, 1}, // 5: line ('\x0a') ->  '\x0a'
     {262, 2}, // 6: line (END) ->  END '\x0a'
     {263, 1}, // 7: expr (NUMBER) ->  NUMBER
     {263, 2}, // 8: expr (uMinus) ->  '-' expr
     {263, 3}, // 9: expr ('+') ->  expr '+' expr
     {263, 3}, // 10: expr ('-') ->  expr '-' expr
     {263, 3}, // 11: expr ('(') ->  '(' expr ')'
     {263, 3}, // 12: expr ('=') ->  expr '=' expr
     {263, 4}, // 13: expr ('(') ->  ident '(' exprList ')'
     {263, 1}, // 14: expr ->  ident
     {265, 3}, // 15: exprList (',') ->  exprList ',' expr
     {265, 1}, // 16: exprList ->  expr
     {264, 1}, // 17: ident (IDENT) ->  IDENT
...
#line 80 "grammar"
        {
         if (vs__(-2).d_type != RuleValue::VARIABLE)
         {
...
#line 89 "grammar"
        {
         if (vs__(-3).d_type != RuleValue::FUNCTION)
         {
         ...
         {
         RuleValue val = vs__(-3).d_function(vs__(-1).d_argList);
         d_val__ = val;
         }
         }
        break;
...
#line 111 "grammar"
        {
         if (vs__(0).d_type != RuleValue::VALUE &&
         vs__(0).d_type != RuleValue::VARIABLE)
         { 
         std::cout << "Argument must resolve to a value";
         d_display = false;
         }
         else
         vs__(-2).d_argList.push_back(vs__(0));
         d_val__ = vs__(-2);
         }
        break;
...
#line 124 "grammar"
        {
         ...
         list.d_argList.push_back(vs__(0));
         d_val__ = list;
         }
        break;
...
#line 134 "grammar"
        {
         ...
         d_val__ = RuleValue(d_constants[d_scanner.matched()]);
         else if (d_functions.find(d_scanner.matched()) != d_functions.end())
         d_val__ = RuleValue(d_functions[d_scanner.matched()]);
