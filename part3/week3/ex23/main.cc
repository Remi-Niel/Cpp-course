...
{
    public:
        // If Needle and first are the same return 1, otherwise add 1 to the result
        // of the next recursion.
        enum{
            located = is_same<Needle, First>::value ?  1 :  
                    Type<Needle, Haystack...>::TypeIdx<ReducedStack...>::located + 1};
};
...
class Type<Needle, Haystack...>::TypeIdx<First>
{
    public:
        enum{
            //If the final member of the haystack was equal to the needle return 1 
            // otherwise return the negated index of the last element. 
            // This way the additions by the other levels in the recursion get cancelled out to 0.
            located = is_same<Needle, First>::value ? 1 : - (sizeof ...(Haystack) - 1)
        };
};
